...
All the lines are identical across facets (toy types).

This is because the model does not include toy_type, so the predicted values are the same for every group.
...
fe_mod (fixed effects): shifts each line up/down based on toy type.

ri_mod (random intercepts): models variation in baseline (intercept) across toy types, but assumes same slope.

rs_mod (random intercepts and slopes): allows both intercept and slope to vary across toy types. Result: different lines for each toy type with different slopes and heights.
```{r}
fe_mod <- lm(R_AGE ~ toy_type + hrs_week, data = toy2)
toy2$pred_fe <- predict(fe_mod)
...
A1 = SD of the distances from each toy-type line to the black (fixed-effect) line → σ₀

A2 = SD of the residuals (observations to group line) → σ_ε

A3 = Where black line cuts the y-axis → fixed effect intercept = γ₀₀

A4 = Slope of the black line → fixed effect slope = b₁
...
A1	σ₀ (SD of random intercepts)
A2	σ_ε (SD of residuals)
A3	γ₀₀ (grand mean intercept)
A4	b₁ (slope of hrs/week)
...
`r qbegin(qcounter())`
How many participants are there in the data?   30
How many have complete data (15 trials)?  0
What is the average number of trials that participants completed? What is the minimum?   12
Does every participant have _some_ data for each type of audio?  yes
...
Outcome of interest: SDMT score

Variables of interest: audio, headphones, and their interaction

Units of observation: individual trials

Clusters/grouping: participants

Within-cluster variation: audio (each participant hears different audio types)

Between-cluster variation: headphones (each participant either wears or doesn’t)
...
We looked at data from 30 people who took an attention test (SDMT) under different sound conditions: no audio, white noise, or music — either through speakers or noise-cancelling headphones. Each person did up to 15 trials, but most had around 12.

We wanted to know:
Does background audio affect performance? And does it matter whether you're wearing headphones?

How we analyzed it
We used a multilevel model to predict SDMT scores based on:

audio type

whether they wore headphones

and the interaction between the two

We also let each person have their own baseline score and their own sensitivity to different audio types.

What we found
Music made people do worse, especially through speakers.

White noise by itself didn’t do much — unless it was played through headphones, where it actually helped.

Wearing headphones, even with no sound, improved performance.

People varied a lot — some benefited from certain audio types, others didn’t.

So what?
Wearing noise-cancelling headphones seems to help attention, even in silence.
Music distracts most people.
And white noise might help — but only when it’s not competing with background noise.

Let me know if you want to make this even more casual (for a talk), or formal (for a paper).
